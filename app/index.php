...
$app -> group('/mesas', function (RouteCollectorProxy $group) {
    $group -> post('[/]', \mesasController::class . ':CargarUno') -> add(new AuthMiddleware(["socio", "mozo"]));
    $group -> post('/cambioEstado', \mesasController::class . ':CambiarEstado') -> add(new AuthMiddleware(["socio","mozo"]));
    $group -> post('/facturacion', \mesasController::class . ':Facturacion') -> add(new AuthMiddleware(["socio","mozo"]));
    ...
    $group -> get('/masUsada', \mesasController::class . ':MesaMasUtilizada')-> add(new AuthMiddleware(["socio"]));
...
$app -> group('/pedidos', function (RouteCollectorProxy $group) {
    ...
    $group -> get('/pendientes', \PedidoController::class . ':ObtenerTodosPedidosPendientes')-> add(new AuthMiddleware(["socio"]));
    ...
    $group -> post('/estadoUnPedido', \PedidoController::class . ':ObtenerEstadoDeUnPedido')-> add(new AuthMiddleware(["socio"]));
    $group -> post('/sector', \PedidoController::class . ':TraerPedidosPendientesPorSector')-> add(new AuthMiddleware(["socio","cocinero","cervecero","bartender","mozo"]));
    $group -> post('[/]', \PedidoController::class . ':CargarUno');//-> add(new AuthMiddleware(["mozo"]));
    $group -> get('/listosParaServir', \PedidoController::class . ':ObtenerTodosPedidosParaServir')-> add(new AuthMiddleware(["mozo"]));
    $group -> post('/cambioEstado', \PedidoController::class . ':CambiarEstadoPedidoProducto')-> add(new AuthMiddleware(["cocinero","cervecero","bartender", "mozo"]));
    ...
    $group -> post('/foto', \PedidoController::class . ':CargarFoto');//-> add(new AuthMiddleware(["mozo"]));
